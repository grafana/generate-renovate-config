...
- Tests use testify's `require` package for assertions.
- Prefer asserting on concrete errors: expect the specific error
  (`require.ErrorContains` with the exact message, or `require.ErrorIs`
  against a sentinel error where one exists) rather than just
  `require.Error`. A test that only checks that *some* error occurred
  keeps passing when the code fails for the wrong reason.
